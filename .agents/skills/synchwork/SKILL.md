---
name: synchwork
description: Synchronize shared maintenance state across the local Posthaste, Clerkwork, Idiolect, Gazetteer, and Patternbook projects.
---

# Synchwork

Use this skill when asked to synchronize, compare, or maintain shared project
state across these five local repositories:
...
from this list just because it is currently empty or mid-bootstrap; report
it as not-yet-ready instead of removing it.
...
1. Confirm the project set.
   - Verify that all five paths exist after expanding `~`.
...
`idiolect`, `gazetteer`, or `patternbook`), plus its `-thumb` variant and any
generated size variants, to the other four repositories.
...
2. Copy each repository's own poster set into the corresponding location in
   the other four repositories, without altering the source repository's own
   image files.
3. Verify that each repository now holds all five projects' poster images
   with matching byte content (compare hashes), for every project that has
...
3. Compare all remaining keys across the five files.
...
5. Apply the agreed common values to all five files, preserving each
...
6. Verify that all keys other than the two excluded theming keys are
   identical across the five files.
...
opens a five-way diff of the local project set:
...
heading, byte-for-byte identical in content across all five repositories.

1. Read each repository's `README.md`.
2. Compare top-level heading structure across the five files and report
...
4. Merge into one canonical version of that section, including an entry for
   ...
   result back into all five `README.md` files.
5. Verify the extracted section is byte-for-byte identical across all five
   repositories.
...
4. Verify all five copies are byte-for-byte identical.
